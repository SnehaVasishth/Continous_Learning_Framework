...
function layoutGraph(g: SgGraph): { nodes: Node[]; edges: Edge[] } {
  ...
  const nodes: Node[] = g.nodes.map((n) => {
    ...
    return {
      id: n.key,
      // dagre gives the box CENTRE; React Flow wants the TOP-LEFT corner.
      position: { x: p.x - NODE_W / 2, y: p.y - NODE_H / 2 },
      data: { label: isTarget ? n.key.replace(/^target:/, "") : n.key },
      // Left-to-right flow: arrows leave a node's right, enter the next's left.
  ...
  const edges: Edge[] = g.edges.map((e) => ({
    ...
    target: e.to,
    type: "smoothstep",
    // Weight is null until observation data exists; show "–" so the absence
    // is explicit rather than blank.
...
export function SignalGraphViewer({ recId }: { recId: number }) {
  ...
  if (err) {
    return <div className="text-sm text-rose-700 dark:text-rose-300 p-3">{err}</div>;
  }
  if (!layouted) {
    return (
      <div className="text-sm text-zbrain-muted dark:text-zbrain-dark-muted p-3">
        Loading graph…
      </div>
    );
  }
  ...
  return (
    <div
      className="rounded-lg border border-zbrain-divider/60 dark:border-zbrain-dark-divider/60 bg-white dark:bg-zbrain-dark-elev1"
      style={{ height: 280 }}
    >
      <ReactFlow
