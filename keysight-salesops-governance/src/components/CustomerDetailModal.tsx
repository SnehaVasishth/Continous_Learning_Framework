  ...
  return (
    <div className="p-6 grid grid-cols-12 gap-4">
      <Block title="Account" cols={6}>
        ...
        <KV k="Customer since" v={d.customer_since?.slice(0, 10) || "n/a"} />
        ...
        <div className="grid grid-cols-3 gap-3">
          {d.addresses.map((a, i) => (
            <div key={i} className="bg-white border border-zbrain-divider rounded-md p-3">
              ...
              <div className="text-sm mt-1">{a.line1 || "n/a"}</div>
            ...
            {d.compliance.map((tag) => (
              <div key={tag} className="bg-white border border-zbrain-divider rounded-md p-2 text-xs">
                ...
                <div className="text-zbrain-muted mt-0.5">
                  {COMPLIANCE_LEGEND[tag] || COMPLIANCE_LEGEND[tag.replace(/\./g, "_")] || "n/a"}
                </div>
              </div>
            ))}
          </div>
    ...
    <div className="p-6 grid grid-cols-2 gap-3">
      {items.map((c) => (
        <div key={c.id} className="bg-white border border-zbrain-divider rounded-lg p-3">
          ...
          <div className="text-xs text-zbrain-muted mt-1">{c.title || "n/a"}</div>
    ...
    <Table headers={["Quote #", "Sales Rep", "Opp ID", "Status", "Valid Until", "Total"]}>
      {items.map((q) => (
        <Row key={q.id}>
          ...
          <Cell>{q.sales_rep || "n/a"}</Cell>
          <Cell mono>{q.opportunity_id || "n/a"}</Cell>
          <Cell>
            <StatusPill status={q.status} />
          </Cell>
          <Cell>{q.valid_until?.slice(0, 10) || "n/a"}</Cell>
    ...
    <Table headers={["Order #", "Status", "Hold", "Ship Date", "Tracking", "CSR", "Total"]}>
      {items.map((o) => (
        <Row key={o.id}>
          ...
          <Cell>{o.requested_ship_date?.slice(0, 10) || "n/a"}</Cell>
          <Cell mono>{o.tracking_number || "n/a"}</Cell>
          <Cell>{o.csr_owner || "n/a"}</Cell>
    ...
    <Table headers={["WO #", "Type", "Status", "Asset S/N", "Scheduled", "Technician"]}>
      {items.map((w) => (
        <Row key={w.id}>
          ...
          <Cell>{w.scheduled_date?.slice(0, 10) || "n/a"}</Cell>
          <Cell>{w.technician || "n/a"}</Cell>
        </Row>
      ))}
    </Table>
  );
    ...
    <Table headers={["Serial", "SKU", "Description", "Location", "Cal Due", "Status"]}>
      {items.map((a) => (
        <Row key={a.id}>
          ...
          <Cell>{a.description || "n/a"}</Cell>
          <Cell>{a.location || "n/a"}</Cell>
          <Cell>{a.calibration_due_date?.slice(0, 10) || "n/a"}</Cell>
          <Cell>
    ...
    <Table headers={["Contract #", "Type", "Starts", "Expires", "Annual Value", "Status"]}>
      {items.map((c) => (
        <Row key={c.id}>
          ...
          <Cell>{c.type}</Cell>
          <Cell>{c.starts_on?.slice(0, 10) || "n/a"}</Cell>
          <Cell>{c.expires_on?.slice(0, 10) || "n/a"}</Cell>
    ...
    <Table headers={["Invoice #", "Issued", "Due", "Currency", "Amount", "Paid", "Status"]}>
      {items.map((i) => (
        <Row key={i.id}>
          ...
          <Cell>{i.invoice_date?.slice(0, 10) || "n/a"}</Cell>
          <Cell>{i.due_date?.slice(0, 10) || "n/a"}</Cell>
    ...
    <Table headers={["Cert #", "Issued", "Expires", "Traceability", "OOT"]}>
      {items.map((c) => (
        <Row key={c.id}>
          ...
          <Cell>{c.issued_date?.slice(0, 10) || "n/a"}</Cell>
          <Cell>{c.expires_date?.slice(0, 10) || "n/a"}</Cell>
          <Cell>
        ...
        <div key={c.id} className="bg-white border border-zbrain-divider rounded-lg p-3">
          <div className="flex items-center gap-2 text-xs">
            <span className="font-mono text-zbrain-muted">
              {c.occurred_at ? new Date(c.occurred_at).toLocaleString() : "n/a"}
            </span>
          ...
          <div className="text-xs text-zbrain-muted mt-0.5">By: {c.sent_by || "n/a"}</div>
